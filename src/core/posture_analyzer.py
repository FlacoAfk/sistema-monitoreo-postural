...
class PostureResult:
    ...
    Fields:
        person_id: Stable per-session person identifier for multi-person routing.
            -1 means "not assigned" (single-person mode or legacy use).
    """
...
class PostureAnalyzer:
    ...
    def __init__(self) -> None:
        # Per-person state tracking for multi-person support
        self._trackers: dict[int, dict] = {}
        # Legacy global tracker (backward compat)
        self._bad_posture_start: Optional[float] = None
        self._last_status: PostureStatus = PostureStatus.NO_DETECTADO
        self._last_update: float = time.time()

    def _get_tracker(self, person_id: int) -> dict:
        """Get or create a state tracker for a specific person."""
        if person_id not in self._trackers:
            self._trackers[person_id] = {
                "bad_posture_start": None,
                "last_status": PostureStatus.NO_DETECTADO,
                "last_update": time.time(),
            }
        return self._trackers[person_id]
    ...
    def analyze(
        ...
        Args:
            ...
            person_id: Identificador de persona para tracking multi-persona.

        Returns:
            PostureResult con CPI, ángulos, curvatura y estado.
        """
        tracker = self._get_tracker(person_id)
        if timestamp is None:
            timestamp = time.time()

        if not detected or len(keypoints) < 9:
            return self._no_detection(timestamp, frame_id, person_id)
        ...
        # ── 7. Contador de tiempo en mala postura (per-person) ────────────────
        now = timestamp
        if status in (PostureStatus.ALERTA_LEVE, PostureStatus.ALERTA_CRITICA):
            if tracker["bad_posture_start"] is None:
                tracker["bad_posture_start"] = now
                bad_accum = 0.0
            else:
                bad_accum = now - tracker["bad_posture_start"]
        else:
            tracker["bad_posture_start"] = None
            bad_accum = 0.0
    ...
    def _no_detection(self, timestamp: float, frame_id: int, person_id: int = -1) -> PostureResult:
        tracker = self._get_tracker(person_id) if person_id >= 0 else None
        if tracker is not None:
            elapsed = timestamp - tracker["last_update"]
            if elapsed > 2.0:
                tracker["bad_posture_start"] = None
            tracker["last_update"] = timestamp
            tracker["last_status"] = PostureStatus.NO_DETECTADO
        else:
